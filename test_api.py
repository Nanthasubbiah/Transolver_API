...
print("=" * 60)
#-------------------------------------------------------------------------------------------------
# Option A: First time? Register to get your key
#-------------------------------------------------------------------------------------------------
...
# print(f"API Key: {api_key}") # Save this API key printed in terminal
# HEADERS = {"X-API-Key": api_key}
#-------------------------------------------------------------------------------------------------
# Option B: Already have your key? Paste it here and skip to Step 2
#-------------------------------------------------------------------------------------------------
# api_key = "your_saved_key_here"
# HEADERS = {"X-API-Key": api_key}
#-------------------------------------------------------------------------------------------------
# Option C: Lost your key? Reset it
#-------------------------------------------------------------------------------------------------
# r = requests.post(f"{BASE}/auth/reset-key", json={"username": "testuser", "password": "testpass"})
# api_key = r.json()["api_key"]
# HEADERS = {"X-API-Key": api_key}
...
if r.status_code == 200:
    ...
    r = requests.post(f"{BASE}/auth/reset-key", json={"username": "testuser", "password": "testpass"})
    api_key = r.json()["api_key"]
    print(f"Reset! New API Key: {api_key}")
...
# Upload node table
nodes = np.load("examples/example_nodes.npy")

# # Optional: load your element table (skip = auto Delaunay)
# elements = np.load("example_elements.npy")
...
r = requests.post(f"{BASE}/solve", headers=HEADERS, json={
    ...
    "bc": {
        ...
        "tractions": [
            {"node_ids": right_nodes, "tx": 1.0, "ty": 0.0},
            {"node_ids": left_nodes, "tx": -1.0, "ty": 0.0}
        ]
    },
    "use_warm_start": True,
    "solver": "petsc",
...
# ── 6. Download displacement results ──────────────────────────────────────────
if r.status_code == 200:
    result = r.json()
    u = np.array(result["displacement"])  # shape (n_nodes, 2)
    
    # Save as .npy
    np.save("displacement_result.npy", u)
    
    # Or as .csv
    np.savetxt("displacement_result.csv", u, delimiter=",", header="ux,uy", comments="")
    
    print(f"Displacement saved! Shape: {u.shape}")
print("\n" + "=" * 60)
